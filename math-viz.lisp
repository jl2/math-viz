...
(defun bisect (primitives function x-min x-max &optional (fx-min nil) (fx-max nil) (depth 10))
  (let* ((f-x-min (if fx-min fx-min (funcall function x-min)))
    ...
    (format t "f(~a) = ~a~%f(~a) = ~a~%f(~a) = ~a~%~%" x-min f-x-min x-mid f-x-mid x-max f-x-max)
    ...
    (let ((change (* f-x-mid f-x-min)))
      (format t "~a~%" change)
      (cond ((or (= change 0.0) (= depth 0))
             x-mid)
            ((< 0.0 change)
             (bisect primitives function x-min x-mid f-x-min f-x-mid (1- depth)))
            (t
             (bisect primitives function x-mid x-max f-x-mid f-x-max (1- depth)))))))
